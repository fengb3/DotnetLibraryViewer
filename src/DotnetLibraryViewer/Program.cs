...
public static class Program
{
    ...
    private static readonly Dictionary<Command, (string desc, string cmd)[]> CommandExamples =
        new();
    ...
    {
        if (!CommandExamples.TryGetValue(command, out var examples))
            return;
        Console.WriteLine();
    ...
    public static async Task<int> Main(string[] args)
    {
        ...
        {
            Description = "Package version (NuGet mode)",
        };
        ...
        {
            Description = "Path to XML documentation file (local DLL mode)",
        };
        var namespaceOption = new Option<string?>("--namespace", "-n")
        {
            Description = "Filter by namespace (wildcard supported)",
        };

        var rootCommand = new RootCommand(
            "dotnet lib-view - .NET Library Viewer: inspect NuGet packages and DLLs"
        );

        #endregion

        #region doc Command
        var docPackageArg = new Argument<string>("package")
        {
        ...
        [
            ...
            (
                "Filter by namespace",
                "dotnet lib-view doc Serilog --package-version 3.1.1 -n Serilog*"
            ),
        ];

        docCommand.SetAction(
                    ...
                    assembly = assembly with
                    {
                        Types = assembly
                            .Types.Where(t =>
                                ...
                                && WildcardMatcher.IsMatch(t.Namespace, nsFilter)
                            )
                            .ToList(),
                    };
                }
        ...
        #region query-type Command
        var qtPackageArg = new Argument<string>("package")
        {
            Description = "NuGet package name or path to a DLL file",
        };
        var qtKeywordOption = new Option<string>("--keyword", "-k")
        {
            Description = "Wildcard pattern to match type names (* and ? supported)",
        };
        ...
        [
            (
                "Wildcard search for types",
                "dotnet lib-view query-type Newtonsoft.Json -k *Serializer*"
            ),
            (
                "Filter by namespace",
                "dotnet lib-view query-type System.CommandLine -k Command -n System.CommandLine*"
            ),
        ];

        queryTypeCommand.SetAction(
            async (parseResult, ct) =>
            {
                ...
                if (nsFilter is not null)
                    types = types.Where(t =>
                        t.Namespace is not null && WildcardMatcher.IsMatch(t.Namespace, nsFilter)
                    );

                var matched = types
                    .Where(t =>
                        WildcardMatcher.IsMatch(t.Name, keyword)
        ...
        #region query-member Command
        var qmPackageArg = new Argument<string>("package")
        {
            Description = "NuGet package name or path to a DLL file",
        };
        var qmKeywordOption = new Option<string>("--keyword", "-k")
        {
            Description = "Wildcard pattern to match member names (* and ? supported)",
        };
        var qmTypeOption = new Option<string?>("--type", "-t")
        {
            Description = "Limit search to a specific type name (wildcard supported)",
        };

        var queryMemberCommand = new Command(
            "query-member",
        ...
        [
            (
                "Search members across all types",
                "dotnet lib-view query-member Newtonsoft.Json -k *Serialize*"
            ),
            (
                "Limit to a specific type",
                "dotnet lib-view query-member System.CommandLine -k *Parse* -t Command"
            ),
        ];

        queryMemberCommand.SetAction(
            async (parseResult, ct) =>
            {
                ...
                {
                    if (
                        nsFilter is not null
                        && (
                            type.Namespace is null
                            || !WildcardMatcher.IsMatch(type.Namespace, nsFilter)
                        )
                    )
                        continue;
                    ...
                    {
                        if (
                            WildcardMatcher.IsMatch(member.Name, keyword)
        ...
        #region detail Command
        var dPackageArg = new Argument<string>("package")
        {
            Description = "NuGet package name or path to a DLL file",
        };
        var dTypeOption = new Option<string>("--type", "-t")
        {
            Description = "Type name to inspect",
        };
        var dMemberOption = new Option<string?>("--member", "-m")
        {
            Description = "Member name to inspect (optional)",
        };
        ...
        [
            ...
            (
                ...
                "dotnet lib-view detail Newtonsoft.Json -t JsonSerializer --include-inherited false"
            ),
        ];

        detailCommand.SetAction(
            async (parseResult, ct) =>
            {
        ...
        #region compare-version Command
        var cvPackageArg = new Argument<string>("package") { Description = "NuGet package name" };
        var cvV1Option = new Option<string>("--version1", "-v1")
        {
            Description = "First version to compare",
        };
        var cvV2Option = new Option<string>("--version2", "-v2")
        {
            Description = "Second version to compare",
        };

        var compareVersionCommand = new Command(
            "compare-version",
        ...
        [
            ...
            (
                "Filter diff by namespace",
                "dotnet lib-view compare-version Newtonsoft.Json -v1 12.0.3 -v2 13.0.3 -n Newtonsoft.Json*"
            ),
        ];

        compareVersionCommand.SetAction(
            async (parseResult, ct) =>
            {
                ...
                {
                    Console.Error.WriteLine(
                        "Error: compare-version requires a NuGet package name, not a DLL path."
                    );
                    return 1;
                }
        ...
        #region list-packages Command
        var lpQueryArg = new Argument<string>("query")
        {
            Description = "Search query (e.g. package name prefix)",
        };
        ...
        var listPackagesCommand = new Command(
            "list-packages",
        ...
        listPackagesCommand.SetAction(
            async (parseResult, ct) =>
            {
                ...
                if (string.IsNullOrWhiteSpace(query))
                {
                    ...
                    Console.Error.WriteLine(
                        "Usage example: dotnet lib-view list-packages Microsoft.Extensions"
                    );
                    return 1;
                }
    ...
    {
        var isLocalDll =
            package.EndsWith(".dll", StringComparison.OrdinalIgnoreCase) && File.Exists(package);
